...
WHERE album.titre LIKE '%".$search."%' OR chanson.nom LIKE '%".$search."%'
ORDER BY nom_artiste ASC");		
	 while($res = $sql->fetch(PDO::FETCH_OBJ)){
			...
			$nom_chanson = str_replace ( '"' , "'" , $nom_chanson );
			$jaquette = $_MUSIC_ROOT.$res->url_pj; 
			 ?>
			 <evenement nom_album="<?php echo $nom_album;  ?>" id_album="<?php echo $id_album;  ?>" nom_chanson="<?php echo $nom_chanson;  ?>" jaquette="<?php echo $jaquette;  ?>" />
			<?php
	 }
?>
<?php echo '</programmation>'; ?>